...
def main(num_chunks, chunk_idx, action='extract_mesh'):
    if action == 'extract_mesh':
        ...
        write_to_json(obj_properties_dict, os.path.join(data_dir, 'mesh_properties{}.json'.format(chunk_idx)))
    ...
    if action == 'save_metadata':
        ...
        for file_name in file_names:
            if file_name.split('.')[0][:-1] == 'mesh_properties':
                obj_properties_dict = load_from_json(os.path.join(data_dir, file_name))
        ...
        df_cats = pd.read_csv(os.path.join(data_dir, 'scannet-labels.combined.tsv'), delimiter='\t')

        # find missing junk categories
        cat_ids = df_metadata['categoryId'].unique()
        ids = set(df_cats['id'].unique())
        junk_ids = []
        for cat_id in cat_ids:
            if cat_id not in ids:
                junk_ids.append(cat_id)

        # remove records with duplicate categoryIds and add remove category for junk meshes.
        def map_id_to_mpcat40(x):
            if x['id'] not in id_to_mpcat40:
                id_to_mpcat40[x['id']] = x['mpcat40']
            else:
                if x['mpcat40'] != id_to_mpcat40[x['id']]:
                    print(x['id'])
                    print(x['mpcat40'])
                    print(id_to_mpcat40[x['id']])
                    raise Exception('Not a duplicate')

        id_to_mpcat40 = {}
        df_cats.apply(lambda x: map_id_to_mpcat40(x), axis=1)
        for junk_id in junk_ids:
            id_to_mpcat40[junk_id] = 'void'
        df_cats = pd.DataFrame.from_dict({'id': id_to_mpcat40.keys(), 'mpcat40': id_to_mpcat40.values()})

        # join metadata with the scannet cat dataframe to obtain the mpcat40 categories.
        df_metadata = df_metadata.merge(df_cats[['id', 'mpcat40']], how='inner', left_on='categoryId',
                                        right_on='id')
        df_metadata = df_metadata.sort_values(by=['room_name', 'objectId'])

        # map each house to train, test and val
        train_houses = read_houses(train_split_path)
        val_houses = read_houses(val_split_path)
        test_houses = read_houses(test_split_path)
        house_to_folder = {}
        ...
        df_metadata['split'] = df_metadata['room_name'].apply(lambda x: house_to_folder[x])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        # parallel -j5 "python3 -u scannet_preprocessing.py {1} {2} {3}" ::: 5 ::: 0 1 2 3 4 ::: extract_mesh
