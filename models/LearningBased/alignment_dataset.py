...
class Scene(Dataset):
    def __init__(self, data_dir, num_queries, mode, alpha, min_num_nbs=5, max_num_nbs=10):
        self.data_dir = data_dir
        self.mode = mode
        self.alpha = alpha
        self.file_names = os.listdir(os.path.join(self.data_dir, self.mode))
        # pick 50 random queries
        np.random.shuffle(self.file_names)
        self.file_names = self.file_names[:num_queries]
    ...
    @staticmethod
    def map_cat_to_objects(query_cats, graph, source_node):
    ...
    def __getitem__(self, idx):
        ...
        query_graph, clean_objects = self.build_query(target_graph, query_node, q_context_objects, self.alpha)
        ...
        data['query_node'] = query_node
        data['query_graph'] = query_graph

        # record the candidates for each context object
        trimmed_bp_graph = {}
        for context_object in bp_graph.keys():
            trimmed_bp_graph[context_object] = {}
            trimmed_bp_graph[context_object]['candidates'] = bp_graph[context_object]['candidates']
        data['bp_graph'] = trimmed_bp_graph
