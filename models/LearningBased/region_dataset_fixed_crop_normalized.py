...
class Region(Dataset):
    def __init__(self, pc_dir, scene_dir, mode, accepted_regions=None, transforms=None, cat_to_idx=None,
                 num_global_crops=2, num_local_crops=8, num_points=4096, save_crops=False, models_dir=None,
                 local_crop_extent=2, global_crop_extent=4, centralize=False):
        self.pc_dir = pc_dir
        ...
        self.global_crop_extent = global_crop_extent
        self.local_crop_extent = local_crop_extent
    ...
    def build_cube_crop(self, pc_bounds, crop_extent, crop_idx, interval_id, grid_res, crop_type=None):
        # find the lower and upper bounds for the starting points of the cube.
        end_points = pc_bounds[1, :]
        beg_points_upper_bound = end_points - crop_extent
        ...
        end_points = beg_points + crop_extent
    ...
    def extract_crops(self, pc, num_crops, crop_extent, idx, pc_bounds=None, grid_res=10, crop_type='local'):
        ...
        for i in range(num_crops):
            ...
            if crop_type == 'local':
                ...
                cube = self.build_cube_crop(pc_bounds, crop_extent, crop_id, interval_id=interval_id,
                                            grid_res=grid_res, crop_type=crop_type)
            else:
                cube = self.build_cube_crop(original_pc_bounds, crop_extent, crop_id, interval_id=interval_id,
                                            grid_res=grid_res, crop_type=crop_type)
            ...
            # normalize the subpc using the dimension of the cube sampling it.
            if self.centralize:
                centroid = np.mean(subpc, axis=0)
                subpc -= centroid
            subpc = subpc / cube['extents'][0]
    ...
    def __getitem__(self, idx):
        ...
        if self.num_global_crops > 0:
            global_crops = self.extract_crops(pc_region, self.num_global_crops, self.global_crop_extent, idx,
                                              crop_type='global')
            if global_crops is None:
                return None
        else:
            # normalize the pc_region
            max_coord = np.max(pc_region)
            global_crops /= max_coord
        ...
        if self.num_local_crops > 0:
            local_crops = self.extract_crops(pc_region, self.num_local_crops, self.local_crop_extent,
