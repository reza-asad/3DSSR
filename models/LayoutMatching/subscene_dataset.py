...
class SubScene(Dataset):
    def __init__(self, scene_dir, pc_dir, metadata_path, accepted_cats_path, max_coord_box, max_coord_scene,
                 file_name_to_idx, mode='train', num_points=4096, num_objects=5, with_transform=False,
                 random_subscene=True, batch_one=True, global_frame=True):
    ...
    def prepare_tensor_data(self, scene, scene_pc, anchor_obj):
        ...
        centroid_data = np.zeros((len(scene_pc), 3), dtype=np.float32)
        ...
        for i, obj in enumerate(objects):
            ...
            # add centroid data
            centroid_data[i, :] = scene_pc[obj]['aabb'].translation
        ...
        return pc_data, centroid_data, label_data, anchor_idx
    ...
    def __getitem__(self, idx):
        ...
        # bring to local frame if necessary.
        ...
        pc, centroid, label, anchor_idx = self.prepare_tensor_data(scene, subscene_pc, anchor_obj)

        # prepare file names for the anchor object.
        file_name_idx = torch.zeros(1)
        file_name_idx[0] = self.file_name_to_idx[file_name]

        data = {'pc': pc, 'centroid': centroid, 'label': label, 'file_name': file_name_idx, 'anchor_idx': anchor_idx}
