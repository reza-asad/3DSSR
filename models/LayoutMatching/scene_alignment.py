...
from engine_seed_alignment import evaluate, train_one_epoch
...
def do_train(
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        if epoch % args.eval_every_epoch == 0 or epoch == (args.max_epoch - 1):
            ...
            loss_dict = {"Validation Loss": validation_loss}
            if is_primary():
                print("==" * 10)
                print(f"Evaluate Epoch [{epoch}/{args.max_epoch}]; Metrics {validation_loss}")
                print("==" * 10)
                logger.log_scalars(loss_dict, curr_iter, prefix="Test/")

            if is_primary() and (
                len(best_val_metrics) == 0 or validation_loss < best_val_metrics["Validation Loss"]
            ):
                best_val_metrics = loss_dict
    ...
    # TODO: change this to per class accuracy.
    loss_dict = {"Validation Loss": validation_loss}
    metric_str = str(loss_dict["Validation Loss"])
    if is_primary():
        print("==" * 10)
        ...
        with open(final_eval, "w") as fh:
            ...
            fh.write(str(best_val_metrics["Validation Loss"]))
            fh.write("\n")
...
def main(local_rank, args):
    ...
    # TODO: change criterion to MSE loss on sine and cosine of the predicted vs gt angle.
    criterion = build_criterion_alignment(args)
