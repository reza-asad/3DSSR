...
def evaluate_net(classifier, valid_loader, cat_to_idx):
    ...
    with torch.no_grad():
        for i, data in enumerate(valid_loader):
            ...
            # apply the model
            output = classifier(pc)[anchor_idx:anchor_idx+1, :]
...
def train_net(args):
    ...
    train_dataset = SubScene(scene_dir=args.scene_dir, pc_dir=args.pc_dir, metadata_path=args.metadata_path,
                             ...
                             random_subscene=args.random_subscene, batch_one=args.batch_one)

    val_dataset = SubScene(scene_dir=args.scene_dir, pc_dir=args.pc_dir, metadata_path=args.metadata_path,
                           accepted_cats_path=args.accepted_cats_path, max_coord_scene=args.max_coord_scene,
                           num_points=args.num_point, num_objects=args.num_objects, mode='val',
...
def main():
    ...
    adjust_paths(args)
    args.batch_one = args.batch_size == 1
